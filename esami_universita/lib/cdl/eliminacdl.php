<?php
    ...
    if(isset($_POST) && isset($_POST['cdl'])) {
        ...
        if (empty($error_msg)) {
            ...
            $sql = "DELETE FROM corso_di_laurea WHERE id = $1";
            ...
            if ($result)
                $success_msg = "Corso di laurea eliminato correttamente.";
            else
...
<form class="uk-form-horizontal" action="<?php echo $pagelink; ?>" method="POST">
    <legend class="uk-legend">Elimina un corso di laurea</legend>
    <legend class="uk-text-small">Ricorda che puoi eliminare solamente corsi di laurea non ancora attivati.</legend>
    <div class="uk-margin">
    ...
    <button class="uk-button uk-button-primary uk-button-danger uk-align-right uk-margin-remove-bottom" <?php if (count($cdl_keys) == 0) {echo 'disabled';} ?>>Elimina</button>
</form>